...
1. **能否让网络显式关注边缘特征？** 标准残差块对纹理和边缘一视同仁，但人眼对边缘锐度更敏感。我们尝试在残差块中加入 Sobel 边缘检测分支（EARB），让网络从一开始就知道边缘在哪里。

2. **能否让网络自动区分通道重要性？** 标准模型中每个输出通道被同等对待，但不同通道的贡献可能不同。我们设计了可学习的通道门控（LCAP），训练完后可以看到哪些通道被压制了。
