...
图像超分辨率（Super-Resolution, SR）旨在从低分辨率（LR）图像重建高分辨率（HR）图像。本文提出 EdgeSR，一种约 1.44M 参数的边缘感知超分辨率模型（×2 超分），通过两个核心模块——Edge-Aware Residual Block（EARB）和 Lightweight Channel Attention Pruning（LCAP）——在标准超分任务中保持与 EDSR Baseline 同等质量的同时，实现了对通道重要性的定量分析。实验表明：EARB 的 Sobel 边缘分支带来了 0.02 dB 的改善；LCAP 揭示了标准 ResBlock 中 37% 的通道存在冗余；几何自集成（Self-ensemble）可在推理时进一步提升 0.1 dB。此外，本文还探索了盲超分辨率（Blind SR）任务，验证了固定参数规模下模型处理真实退化的能力边界。
...
单图像超分辨率（Single Image Super-Resolution, SISR）的目标是从低分辨率图像恢复高分辨率图像，是计算机视觉中的经典问题。本项目基于 EDSR [1] 架构，设计了一个约 1.44M 参数的轻量超分辨率模型 EdgeSR，主要关注两个问题：

1. **能否让网络显式关注边缘特征？** 标准残差块对纹理和边缘一视同仁，但人眼对边缘锐度更敏感。本项目尝试在残差块中加入 Sobel 边缘检测分支（EARB），让网络从一开始就知道边缘在哪里。

2. **能否让网络自动区分通道重要性？** 标准模型中每个输出通道被同等对待，但不同通道的贡献可能不同。本项目设计了可学习的通道门控（LCAP），训练完后可以看到哪些通道被压制了。
...
本项目参考 Enhanced Deep Residual Networks（EDSR, CVPR 2017）[3] 的简化版本作为基线模型（×2 超分）。
...
### 2.2 EARB（Edge-Aware Residual Block）
...
每个 EARB 比 ResBlock 多 **8,192** 个参数（约 11%）。但由于本项目只将后 8 个 ResBlock 替换为 EARB，且 EdgeSR 的总参数量仍与 Baseline 相同（通过其他方式平衡），整体参数量保持不变。

### 2.3 LCAP（Lightweight Channel Attention Pruning）
...
|------|--------|------|-------|--------|
| Bicubic（双三次插值）| — | 33.66 / 0.930 | 30.24 / 0.869 | 29.56 / 0.843 |
| **EDSR Baseline（EDSR简化版）** | **1.44M** | **35.86 / 0.951** | **31.64 / 0.906** | **30.86 / 0.902** |
| **EdgeSR（本文）** | **1.44M** | **35.85 / 0.951** | **31.52 / 0.904** | **30.84 / 0.902** |
| EDSR 原版 [3] | 43M | 38.11 / 0.960 | 33.92 / 0.920 | 32.32 / 0.901 |

（Bicubic 和 EDSR 原版数据均来自原论文 [3]，其中 Bicubic 为双三次插值基线，EDSR 原版为 43M 参数的完整模型。本文的简化版在 1.44M 参数规模下探索架构与通道重要性。）
...
- **LCAP 的成本**：配置 ③（完整 EdgeSR）相比配置 ② 下降了约 0.03 dB，说明 LCAP 的门控机制带来了一定的信息损失——部分通道被软抑制后，网络无法完全补偿。但这一微小损失的"回报"是通道重要性分析的能力（见 4.5 节）。

### 3.5 LCAP 门控分布分析

#### 3.5.1 统计结果

本项目对训练好的 EdgeSR 模型中所有 16 个 LCAP 层的 1,024 个门控值进行了全面分析：
...
这些发现对模型压缩有直接指导意义：如果采用更温和的渐进式剪枝策略（如每一轮迭代剪枝 10% + 微调 50 epoch），有望在保持质量的同时实现有效压缩，本项目因时间限制未进一步验证。

### 3.6 剪枝实验

基于 LCAP 门控分析的结果，本项目尝试了硬门控剪枝——将训练好的 EdgeSR 中的 LCAP 层替换为固定的二值掩码（BinaryLCAP），对低于阈值的通道永久置零，然后微调 200 epoch 以恢复质量。
...
本项目通过钩子（Hook）提取了 8 个 EARB 块的 Sobel 边缘分支输出，使用 Set5 数据集中 butterfly 图像进行可视化。
...
本项目计算了 EdgeSR 和 Baseline 输出图像的逐像素差异（绝对值平均，放大 10 倍以增强可见性）。

**图3：EdgeSR vs Baseline 对比。** 从左至右依次为：LR 输入、EdgeSR 输出、Baseline 输出、差值图（|EdgeSR − Baseline|，×10 放大）、HR 真值。

![sr_comparison](..\results\sr_comparison.png)
...
真实世界的图像退化远比双三次下采样复杂。为了探索固定参数规模下模型在盲 SR 场景的能力边界，本项目进行了多轮实验。
...
本项目的退化实验虽然未能在标准 benchmark 上达到与 clean 训练相同的水平，但证明了以下几点：
...
为了直观展示不同模型的超分效果，选取了两张测试图像：

- **comic**（Set14）：标准基准图像，用于对比 Baseline 与 EdgeSR 的 SR 输出差异
- **zhenshi**（真实模糊照片）：用手机拍摄，用于对比不同退化训练模型在真实世界图像上的表现

**图4：Set14/comic 的 SR 效果对比。** 左：LR 输入（Bicubic 放大），右：EdgeSR 标准模型输出。

![comic LR vs EdgeSR](../tmp/comic_compare.png)

**图5：真实模糊照片 SR 效果（LR vs 标准 EdgeSR）。**

![LR vs 标准 EdgeSR](../tmp/zn_standard_compare.png)

**图6：真实模糊照片 SR 效果（LR vs 退化模型）。**

![LR vs 退化模型](../tmp/zn_degrad_compare.png)

**图7：真实模糊照片 SR 效果（LR vs DRealSR 微调）。**

![LR vs DRealSR 微调](../tmp/zn_drealsr_ft_compare.png)

**图8：真实模糊照片 SR 效果（LR vs SSIM 模型）。**

![LR vs SSIM 模型](../tmp/zn_ssim_compare.png)

从 comic 的对比可以看出，EdgeSR 标准模型输出的线条更清晰、边缘更锐利，相比 Bicubic 放大有明显改善。
...
此外，本项目还基于 Gradio 搭建了网页推理界面（src/gradio_app.py），支持上传图片并实时查看超分结果，便于直观对比不同模型的输出效果。具体使用请参考附录。

---

## 4. 思考与总结

### 4.1 为什么 EdgeSR 没有显著超过 Baseline？

这是一个需要诚实回答的问题。我认为有以下原因：
...
1. **EARB 模块**：通过 Sobel 边缘分支显式引导网络关注边缘特征，消融实验显示带来了 0.02 dB 的提升。

2. **LCAP 模块**：以极低的参数代价（每层 64 个标量）实现了通道重要性分析，门控分布显示 37% 的通道被抑制超过 50%。

3. **剪枝实验**：验证了硬门控剪枝的可行性边界——3% 剪枝几乎无损。

4. **盲 SR 探索**：尝试了退化增强、真实数据、二阶退化和 L1+SSIM 损失，验证了模型在退化场景下的能力边界。

### 5.2 预训练权重

本文所有实验的预训练权重文件可通过百度网盘下载：https://pan.baidu.com/s/1IMmuZ5YzPBbe4phtFLQyVw?pwd=zeuj （提取码：zeuj）
...
# Gradio demo
python -m src.gradio_app --config configs/edgesr_standard.yaml --model edgesr --checkpoint checkpoints/edgesr_standard_best.pt --port 7860
```
