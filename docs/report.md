...
图像超分辨率（Super-Resolution, SR）旨在从低分辨率（LR）图像重建高分辨率（HR）图像。本文提出 EdgeSR，一种 1.44M 参数的可解释性边缘感知超分辨率模型，通过两个核心模块——Edge-Aware Residual Block（EARB）和 Lightweight Channel Attention Pruning（LCAP）——在标准超分任务中保持与 EDSR Baseline 同等质量的同时，实现了对通道重要性的可解释性分析。实验表明：EARB 的 Sobel 边缘分支带来了 0.02 dB 的改善；LCAP 揭示了标准 ResBlock 中 37% 的通道存在冗余；几何自集成（Self-ensemble）可在推理时进一步提升 0.1 dB。此外，本文还探索了盲超分辨率（Blind SR）任务，验证了固定参数规模下模型处理真实退化的能力边界。
...
2. **LCAP（Lightweight Channel Attention Pruning）**：为每个通道学习一个可训练的门控值，在训练中通过软抑制自动区分通道重要性，为后续模型压缩提供可解释性依据。
...
真实世界的 LR 图像通常包含多种退化（模糊、噪声、JPEG 压缩），而非简单的双三次下采样。BSRGAN [12] 提出了"先下采样再退化"的盲 SR 流水线；Real-ESRGAN [6] 使用二阶退化进一步提高退化多样性。本项目的盲 SR 探索主要参考了这些工作。
...
（作为对比，EDSR 原版参数量为 43M，远大于本文的 1.44M 版本。本文的简化版在较小参数规模下探索架构可解释性。）
...
| ② Baseline + EARB（8个EARB替换8个ResBlock）| 1.44M | **35.88** | **31.62** | **30.85** |
| ③ Baseline + EARB + LCAP（完整EdgeSR）| 1.44M | 35.85 | 31.52 | 30.84 |

**消融结论：**

- **EARB 的贡献**：配置 ② 在 Set5 上达到 35.88，超过 Baseline（35.86），说明 EARB 确实带来了正向提升。尽管提升幅度很小（0.02 dB），但在竞争激烈的 SR 基准上，在不增加参数量的前提下实现任何提升都是有意义的。
- **LCAP 的成本**：配置 ③ 相比配置 ② 下降了约 0.03 dB，说明 LCAP 的门控机制带来了一定的信息损失——部分通道被软抑制后，网络无法完全补偿。但这一微小损失的"回报"是通道重要性的可解释性（见 4.5 节）。
...
| body.17 | EARB | 0.533 | 0.439 | 0.675 | 0 | 25 |
| body.19 | EARB | 0.531 | 0.449 | 0.686 | 0 | 21 |
| body.21 | EARB | 0.517 | 0.432 | 0.713 | 0 | 30 |
| body.23 | EARB | 0.500 | 0.368 | 0.718 | 0 | 36 |
| body.25 | EARB | 0.480 | 0.377 | 0.730 | 0 | 43 |
| body.27 | EARB | 0.461 | 0.359 | 0.725 | 0 | 53 |
| body.29 | EARB | 0.421 | 0.318 | 0.687 | 0 | 60 |
| body.31 | EARB | **0.301** | **0.174** | 0.484 | **33** | **64** |
...
（附图：sobel_edges.png，展示 8 个 EARB 块经过 LCAP 门控后的输出特征）

观察发现，从浅层到深层 EARB，经过门控后的特征响应呈现递减趋势：
- 浅层 EARB 输出的边缘特征较为清晰完整
- 深层 EARB 的输出逐渐减弱，最后几层的边缘信息明显减少
- 这一趋势与 LCAP 门控分析一致——深层 EARB 的门控值最低，对输出特征的衰减最强

值得注意的是，Sobel 边缘检测分支（固定核）本身的输出在所有层中保持稳定。LCAP 门控作用的是整个 EARB 的融合输出（纹理 + 边缘），而非 Sobel 分支单独的输出。因此门控值递减反映的是网络对 EARB 整体输出的置信度降低，而非边缘检测能力的退化。

#### 4.7.2 差值图分析

我们计算了 EdgeSR 和 Baseline 输出图像的逐像素差异（绝对值平均，放大 10 倍以增强可见性）。

（附图：sr_comparison.png，包含 LR 输入 / EdgeSR 输出 / Baseline 输出 / 差值图(×10) / HR 真值）
...
> - 部分白色斑点边界
>
> 橙色翅面与黑色静脉的交界处，而背景纯色区域（大块橙色、粉色花朵）几乎全黑，差异接近零。
>
...
3. **PNSR 的局限性**：PSNR 是基于像素级差异的指标，对边缘锐度、纹理质量等感知因素不敏感。EdgeSR 的差值图分析显示，两个模型在边缘区域存在系统性的微小差异，但 PSNR 无法反映这种差异。这也是 SR 领域近年来越来越关注感知指标（如 LPIPS、MOS）的原因。

### 5.2 LCAP 的实际价值

LCAP 的核心价值不在于提升 PSNR，而在于**提供了一个零成本的可解释性工具**。通过 LCAP 的门控值，我们可以：
- 量化每个通道的相对重要性
- 定位冗余通道的位置和分布
- 为模型压缩提供数据驱动的决策依据
...
本文提出了 EdgeSR，一种可解释性的边缘感知超分辨率模型。主要成果包括：

1. **EARB 模块**：通过 Sobel 边缘分支显式引导网络关注边缘特征，在消融实验中带来 0.02 dB 的提升。差值图分析确认了 EARB 的输出差异集中在图像边缘区域。

2. **LCAP 模块**：以极低的参数代价（每层 64 个标量），实现了通道重要性的可解释性分析。门控分布显示 37% 的通道被抑制超过 50%。
...
[11] Guo, J., et al. "Gate Decorator: Global Filter Pruning Method for Accelerating Deep Convolutional Neural Networks." (NeurIPS 2019)
