...
# 测试集 — 从 HuggingFace datasets 下载（eugenesiow 提供 SR benchmark 图片）
# 在国内通常可以访问 huggingface.co
BENCHMARK_HF_BASE = "https://huggingface.co/datasets/eugenesiow"
...
def main():
    ...
    if not args.skip_download:
        ...
        if not args.skip_benchmark:
            print("\n=== Downloading Benchmark Sets (HuggingFace eugenesiow) ===")
            for name, files in BENCHMARK_FILES.items():
                print(f"  Downloading {name} ({len(files)} images)...")
                hf_url = f"{BENCHMARK_HF_BASE}/{name}/resolve/main/data"
                for fname in files:
                    url = f"{hf_url}/{fname}.png"
                    save_path = os.path.join(args.benchmark_root, name, f"{fname}.png")
                    download_file(url, save_path)
    ...
    # Benchmark sets are downloaded as individual PNG files — no extraction needed
