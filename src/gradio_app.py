"""
EdgeSR Gradio 网页推理界面。

用法：
    python -m src.gradio_app --config configs/edgesr_standard.yaml --model edgesr --checkpoint checkpoints/edgesr_standard_best.pt
"""
...
def process_image(model, input_img, scale_factor, device):
    ...
    参数：
        model：PyTorch 模型
        input_img：PIL 图像或 numpy 数组
        scale_factor：放大倍数（2 或 4）
        device：torch 设备

    返回：
        (input_display, output_pil)：显示用的图像对
    """
